...
def setup():
    executable_path = r'C:\Users\yeyuc\Desktop\GitHub_C2C\patent_country_innovation\C2C\enforcement\chromedriver.exe'
...
def searchOnline(browser, target):
    ...
    time.sleep(3)
    browser.find_element_by_xpath('//*[@id="MapHolder"]/div[1]/div[1]').click()
...
def findLatLong(browser, target_list):
    ...
    for item in target_list:
        ...
        if item in CountryLabelLocation:
            latlonglist.append(CountryLabelLocation[item])
        elif item in ChineseCityLocation:
            latlonglist.append(ChineseCityLocation[item])
        else:
            try:
...
def main():
    ...
    for data in data_list:
        try:
            source_data = pd.read_csv(source_path + '\\' + data, encoding="GBK")
        except:
            source_data = pd.read_csv(source_path + '\\' + data, encoding="UTF-8")
        ...
        source_data.to_csv(target_path + '\\' + data, index=False, encoding='GBK')
...
if __name__ == '__main__':
    """
    基于selenium，从（http://api.map.baidu.com/lbsapi/getpoint/index.html）百度地图坐标拾取系统，
    爬取131个国家、131个国家的首都、中国34个行政区省会和中国351个城市的经纬度坐标
    （部分坐标已知，未知部分在线爬取）
    """
